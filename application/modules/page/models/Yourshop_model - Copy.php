...
class Yourshop_model extends CI_Model 
{
	...
	public function updateproductvariations($productid) 
	{
		
		// Color Variation Option
		
		$coloroptgroupid = $this->input->post('coloroptiongroupid');
		...
		if( count($this->input->post('coloroptionname')) > $colorVaritionGet->num_rows()){
			...
			for($opt=0;$opt<count($this->input->post('coloroptionname'));$opt++){
				...
				$sizeoptiongroupid = $this->input->post('coloroptiongroupid');
				$sizeoptionname = $this->input->post('coloroptionname');
		...
		$sizeoptgroupid = $this->input->post('sizeoptiongroupid');
	...
	public function insertproductvariationsColor($productid) 
	{
		...
		for($opt=0;$opt<count($this->input->post('coloroptionname'));$opt++){
			...
			$data = array(
				'productid'			=> $productid,
				'optiongroupid'		=> $option_group_nameC[$opt],
	...
	public function getrenewal() 
	{
		...
		for($od=0;$od<count($this->input->post('renewalpid')); $od++){
			...
			$getProductRenewalMonth = $this->db->query("SELECT product_renewal FROM mega_settings");
			extract($getProductRenewalMonth->row_array());
					
			$renewalMonth = $product_renewal;
			
			$update_products = array(
				'product_update_date' 	=> $currentdate,
				'product_expire_date' 	=> addmonths($renewalMonth),
				'bill_paid_or_not' 		=> 1,
