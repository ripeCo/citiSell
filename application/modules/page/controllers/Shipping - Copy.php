...
class Shipping extends CI_Controller 
{
	...
	public function tracking($trackingNumber) {
		...
		$jsonResponse = $t->track($trackingNumber);
	...
	*/
	private function confirmShipment($orderId, array $shopOwner, array $buyer, array $packageParam, array $addOnsParam=null) {
		...
		$shipment->setParameter('toState', $buyer['user_state']);
		...
		$rate->setParameter('insuredValue', $packageParam['order_amount']);
		...
		if (isset($addOnsParam['signatureType'])) {
			$a = new \stdClass();
	...
	*/
	public function confirmAndBuy() {
		...
		$packageParam['order_amount'] = $orderDetails[0]['order_amount'];
	...
	/* 	Description - We always assume data being passed are all been sanitized already. Meaning no more conditional statements.
		...
		@return  - returns response from RocketShipIt
	*/
	private function calculateShippingRates2(array $shopOwner, array $buyer, array $packageParam, array $addOnsParam=null) {
		$this->load->helper('myhelp');
		...
		/*if(isset($packageParam['insuredValue']))
			$rate->setParameter('insuredValue', $packageParam['insuredValue']); */
		...
		if (isset($addOnsParam['insuredValue'])) {
			$a = new \stdClass();
			$a->AddOnType = 'SC-A-INS';
			array_push($addons, $a);
		}
		if (isset($addOnsParam['signature'])) {
			$a = new \stdClass();
			$a->AddOnType = 'US-A-SC';	// Include Signature Confirmation service.
			array_push($addons, $a);
		}
	...
	private function calculateShippingRates(array $shopOwner, array $buyer, array $packageParam, array $addOnsParam) {
		...
		$config->setDefault('generic', 'shipper', $shopOwner['shipper']);
		$config->setDefault('generic', 'fromName', $shopOwner['shipper']);
		$config->setDefault('generic', 'shipContact', $shopOwner['user_phone']);
		$config->setDefault('generic', 'shipPhone', $shopOwner['user_phone']);
		$config->setDefault('generic', 'shipAddr1', $shopOwner['user_address']);
		$config->setDefault('generic', 'shipAddr2', $shopOwner['shipAddr2']);
		$config->setDefault('generic', 'shipCity', $shopOwner['user_city']);
		...
		if (isset($packageParam['length']) && isset($packageParam['length']) && isset($packageParam['length'])) {
			$rate->setParameter('length', $packageParam['length']);
			$rate->setParameter('width', $packageParam['width']);
			$rate->setParameter('height', $packageParam['height']);
		}
		...
		file_put_contents('c:\tmp\calculateShippingRatesResponse.txt', print_r($shippingRates, true));
	...
	*/
	private function prepareCalculatingShipmentRates(array $data) {
		...
		if(isset($data['signature']))
			$addOnsParam['signature'] = $data['signature'];
