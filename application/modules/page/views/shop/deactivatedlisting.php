                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="user_name"><!-- Begin: user_name -->
						...
						<h3 class="user_name_h3 pull-left">
							...
							Deactivated product Listings
							
							<span class="text-primary">
								<?php
									$sid = $this->session->userdata('shopid');
									
									$numofproducts = $this->db->query("select * from mega_products where shopid=$sid and product_live='Inactive'");
                                                ...
                                                <div class="ctsell_items" id="filter"><!-- Begin: ctsell_items -->
                                                    <?php
														...
														foreach($obj as $viewws)
														{
															...
															$filters = $this->db->query("SELECT distinct {$viewws->Field} FROM $table_name where shopid=$shopid and product_live='Inactive'");
															$obj1 = $filters->result();
	...
	<script>
		...
		var main_opts;
		   ...
		   var tbl_body = "";
			  $.each(data, function() {
				...
				$.each(this, function(k , v) {
					...
					if(k=='productid'){
						tbl_row += "<div class='pedit'><a onclick='return confirmActive();' class='btn btn-primary' href='<?php echo base_url(); ?>page/yourshop/activeproduct/"+v+"' title='Active product'><i class='fa fa-pencil' aria-hidden='true'></i> Active</a></div>";
					}
		  ...
		  function update(){
			$.ajax({
			  type: "POST",
			  url: "<?php echo base_url(); ?>page/yourshop/shoplistinginactivatedview",
			  dataType : 'json',
			  cache: false,
