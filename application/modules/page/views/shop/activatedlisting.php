						...
						<h3 class="user_name_h3 pull-left">
							...
							<span class="text-primary">
								<?php
									...
									$numofproducts = $this->db->query("select * from mega_products where shopid=$sid and product_live='Active'");
                                                ...
                                                <div class="ctsell_items" id="filter"><!-- Begin: ctsell_items -->
                                                    <?php
														...
														foreach($obj as $viewws)
														{
															...
															$filters = $this->db->query("SELECT distinct {$viewws->Field} FROM $table_name where shopid=$shopid and product_live='Active'");
															$obj1 = $filters->result();
	...
	<script>
		...
		var main_opts;
		   ...
		   var tbl_body = "";
			  $.each(data, function() {
				...
				$.each(this, function(k , v) {
					...
					if(k=='productid'){
						tbl_row += "<div class='pedit'><a onclick='return confirmDeactive();' class='btn btn-primary' href='<?php echo base_url(); ?>page/yourshop/deactiveproduct/"+v+"' title='Active product'><i class='fa fa-pencil' aria-hidden='true'></i> Deactive</a></div>";
					}
		  ...
		  function update(){
			$.ajax({
			  type: "POST",
			  url: "<?php echo base_url(); ?>page/yourshop/shoplistingdeactivatedview",
			  dataType : 'json',
			  cache: false,
