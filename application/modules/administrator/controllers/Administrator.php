...
class Administrator extends CI_Controller 
{
	...
	public function form()
	{
		...
		else 
		{
			...
			if($check == TRUE) 
			{
				$this->session->set_userdata('isAdminLogin', TRUE); // This is a public session that used every controller
   ...
   /* public function rolekey_exists($key) {
      ...
      if($this->login_model->mail_exists($key)){
		...
		$emailfrom           = sitename(). emailfrom();
		$replyemail          = emailreplyto();
		$recipemail          = $this->input->post('useremail');
        
        $this->smtpmail('rony@wanitbd.com','Password recovered!',$msg,$emailfrom,$replyemail,'http://citisell.com/demo/assets/frontend/images/interface/logo.png');
	...
	$recipemail          = $this->input->post('useremail');
        ...
        $this->smtpmail($key,'Password recovered!',$msg,'citisell@citisell.com','citisell@citisell.com','http://citisell.com/demo/assets/frontend/images/interface/logo.png');
	...
	public function smtpmail($receipent,$subj,$msg,$from,$reply,$attach)
	{
		...
		<h4>Regards,</h4>
		<p>
			<a href="http://www.citisell.com" targer="_blank" title="CitiSell">CitiSell</a>
		</p>
	...
	public function dbBackup(){
		...
		$dbname = 'ctSell_shopPortal_dbBackup_'.date('d-m-Y_H_i_s');
