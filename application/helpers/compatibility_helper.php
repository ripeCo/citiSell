 ...
 */
 ...
 // Create User Log
	function hasAccess($currentpage,$actions,$username,$password,$fullname,$attampts){
 ...
 if ( ! function_exists('file_put_contents'))
{
	function file_put_contents($filename, $data, $flags = NULL)
	{
		...
		{
			$backtrace = debug_backtrace();
			_exception_handler(E_USER_WARNING, 'file_put_contents('.htmlentities($flder.$filename).') failed to open stream', $backtrace[0]['file'], $backtrace[0]['line']);
			return FALSE;
		}
		...
		if (($flags & LOCK_EX) > 0)
		{
			...
			{
				$backtrace = debug_backtrace();
				_exception_handler(E_USER_WARNING, 'file_put_contents('.htmlentities($flder.$filename).') unable to acquire an exclusive lock on file', $backtrace[0]['file'], $backtrace[0]['line']);
				return FALSE;
			}
		}
		...
		{
			$backtrace = debug_backtrace();
			_exception_handler(E_USER_WARNING, 'file_put_contents('.htmlentities($flder.$filename).') failed to write to '.htmlentities($flder.$filename), $backtrace[0]['file'], $backtrace[0]['line']);
		}
